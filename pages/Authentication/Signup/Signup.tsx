...
import { validatePassword, validateUsername } from '../../../controllers/validation';
...
const Signup = ({ navigation }:Props) => {
  ...
  const handleSignup = ()=> {
    if(!validateUsername(userName.value,2)) {
      setUserName((prev:any)=>({...prev,isError:true,error:"Invalid Username"}))
    }
    if(!validatePassword(pass.value,2)) {
      setPass((prev:any)=>({...prev,isError:true,error:"Invalid Password"}))
    }
    else {
       ...
       <Text style={[globalStyles.boldText,styles.title]}>Signup</Text>
        <View style={styles.formBox}>
          <TextInput
            ...
            onChangeText={(userName: string) => setUserName((prev: any) => ({ ...prev, value: userName }))}
        ...
        {pass.error}
          ...
          <Button labelStyle={{fontFamily:"Poppins-Regular"}} rippleColor={secondaryBg} mode="contained" style={styles.loginBtn} onPress={handleSignup}>Next</Button>
        </View>
